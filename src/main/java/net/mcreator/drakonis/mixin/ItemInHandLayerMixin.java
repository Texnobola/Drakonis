package net.mcreator.drakonis.mixin;
...
import net.mcreator.drakonis.DrakonisModPlayerAnimationAPI;
...
public abstract class ItemInHandLayerMixin {
	...
	private void animateItem(LivingEntity livingEntity, ItemStack itemStack, ItemDisplayContext transformType, HumanoidArm arm, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, CallbackInfo ci) {
		...
		if (livingEntity instanceof Player player) {
			...
			if (animation == null)
				return;
			DrakonisModPlayerAnimationAPI.PlayerBone bone = animation.bones.get(arm == HumanoidArm.LEFT ? "left_item" : "right_item");
			if (bone == null)
				return;
			...
			Vec3 rotation = DrakonisModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
